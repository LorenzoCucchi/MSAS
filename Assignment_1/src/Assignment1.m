...
[t12, sol12, n12, fcnE12, c12] = newt2Dfinforw(func, x0, toll);
[t13, sol13, n13, fcnE13, c13] = newt2Dfincent(func, x0, toll);

% Finding the second zero z2
x0 = [6 5]';
% Analytical method, netwon finite forward, newton finite centered
[t21, sol21, n21, fcnE21, c21] = newt2Da(func, x0, invJ, toll);
[t22, sol22, n22, fcnE22, c22] = newt2Dfinforw(func, x0, toll);
[t23, sol23, n23, fcnE23, c23] = newt2Dfincent(func, x0, toll);
...
%%% POINT 3 ---------------------------------------------------------------
alphaVec = [linspace(0, 75*pi/180, 1/10*N),...
            linspace(75*pi/180, pi/2, 8/10*N),...
            linspace(pi/2, pi, 1/10*N)];
...
% function and the definition of inputs and outputs.
%
% Every function returns the time of execution, this is done inside of the
% function in order to evaluate only the algorithm and execlude time used
% for the function call.

%%% ------------------------------ EX-1 ---------------------------------

function [t, sol, it, fcnE, cV] = newt2Da(f, x, invJ, toll)
% newt2Da - computes the zero of a multidimensional function described by
% the function handle f, using Newton's method with an analytical inverse 
% Jacobian matrix specified by the function handle invJ.
%
% PROTOTYPE
...
function [t, sol, it, fcnE, cV] = newt2Dfinforw(f, x, toll)
% newt2Df - computes the zero of a multidimensional function described by 
% the function handle f, utilizing Newton's method and finite forward 
% differences for the computation of the Jacobian matrix.
%
% PROTOTYPE
%   [timeComp, sol, it, fcnE, cV] = newt2Dfinforw(f, x, toll)
%
% INPUT:
%   f        fcn handle  [1x1]   function to evaluate                   [-]
%   x        double      [nx1]   starting guess                         [-]
...
function [t, sol, it, fcnE, cV] = newt2Dfincent(f, x, toll)
% newt2Df - % newt2Df - Function to compute the zero of the multidimensional
% function described by f applying Newton's method and centered finite 
% differences approximation for the computation of the Jacobian matrix
%
% PROTOTYPE
%   [timeComp, sol, it, fcnE, cV] = newt2Dfincent(f, x, toll)
%
% INPUT:
%   f        fcn handle  [1x1]   function to evaluate                   [-]
%   x        double      [nx1]   starting guess                         [-]
...
% FRK4 - Function to retrieve the linear operator of the second order
% Runge-Kutta approximation RK4.
%
% PROTOTYPE
%   F = FRK4(h, alpha)
...
function addAxes(ax)
% addAxes - Function to plot axis with scalatures
%
% PROTOTYPE
%   [N, H] = getNH(ax)
%
% INPUT:
%   ax          axis [1x1]     handle to current axis                   [-]
%
% OUTPUT: [-]
%
% CREATOR:
